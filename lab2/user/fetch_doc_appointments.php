...
if (isset($_GET['username'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $check="SELECT fname,lname FROM `patreg` WHERE email='$email'";
            $check_query=mysqli_query($con, $check);
            $check_row = mysqli_fetch_assoc($check_query);
            $pname = $check_row['fname'] . " " . $check_row['lname'];
                    ...
                    <tr>
                        <form id='uploadForm_<?php echo $row['id']; ?>' method='post' action='book.php' enctype='multipart/form-data'>
                            ...
                            if ($row['paid'] != '0') { 
                                ...
                                <td class='text-center'><button  class="btn btn-success" onclick="payed(
                                    '<?php echo htmlspecialchars($row['test_id']); ?>',
                                    '<?php echo htmlspecialchars($row['price']); ?>'
                                )">Pay now</button></td>
                            <?php } ?>
